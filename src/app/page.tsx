...
const nav = [['Home','#top'],['Demo','/demo'],['App','/app'],['For Owners','/owner'],['Contact','#contact']];
...
export default function HomePage(){
  ...
  return <main className="fm-site" id="top">
    <header className="fm-header">
      ...
      <nav className="fm-nav" aria-label="Main navigation">{nav.map(([label,href])=><a key={label} href={href}>{label}</a>)}</nav>
      <a className="fm-header-cta" href="/demo">Get Started <span>→</span></a>
      <button className="fm-menu" type="button" aria-label={menuOpen?'Close menu':'Open menu'} aria-expanded={menuOpen} onClick={()=>setMenuOpen(v=>!v)}><span/><span/><span/></button>
    </header>
    {menuOpen&&<div className="fm-mobile-menu" role="dialog" aria-label="Mobile navigation"><div className="fm-mobile-menu-inner">{nav.map(([label,href])=><a key={label} href={href} onClick={()=>setMenuOpen(false)}>{label}<span>↗</span></a>)}<a className="fm-mobile-cta" href="/demo" onClick={()=>setMenuOpen(false)}>Get Started <span>→</span></a></div></div>}
    ...
    <section className="fm-built"><div className="fm-built-panel"><div className="fm-section-heading"><h2>Built for Modern Industry</h2><p>FactoryMind combines cutting-edge AI with real-world industrial expertise<br className="desktop-only"/> to deliver measurable results.</p></div><div className="fm-feature-grid">{features.map(([icon,title,desc])=><article key={title}><span className="fm-feature-icon">{icon}</span><h3>{title}</h3><p>{desc}</p></article>)}</div></div></section>
    <section className="fm-lower" id="contact"><a href="/demo"><span>/demo</span><b>Live Demo</b><small>Explore FactoryMind in action.</small><em>↗</em></a><a href="/app"><span>/app</span><b>The FactoryMind App</b><small>Your factory. In your hands.</small><em>↗</em></a><a href="/owner"><span>/owner</span><b>For Factory Owners</b><small>More control. A smarter future.</small><em>↗</em></a></section>
